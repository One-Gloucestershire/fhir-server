...
export class ServiceAccountHandler implements Authorization {
    ...
    private readonly customServiceScope =
        process.env.CUSTOM_OAUTH2_SERVICE_ACCOUNT_SCOPE || 'custom-fhir-scope/default';

    constructor(rules: RBACConfig, fhirVersion: FhirVersion) {
        this.rules = rules;
        if (this.rules.version !== this.version) {
            throw new Error('Configuration version does not match handler version');
        }
    ...
    async verifyAccessToken(request: VerifyAccessTokenRequest): Promise<KeyValueMap> {
        ...
        if (decoded.token_use === 'access' && decoded.scope === this.customServiceScope) {
            groups.push('practitioner', 'non-practitioner', 'auditor');
            decoded['cognito:groups'] = groups;
        }
